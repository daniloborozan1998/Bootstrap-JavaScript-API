...
html.btnSpanish.addEventListener("click", function() {
    html.cardContainer.innerHTML = "";
    html.notification.innerHTML = "";

    promise.then(function(dataFromFile) {
        let results = dataFromFile.filter(x => x.languages[0].name === "Spanish");
        results.forEach(country => {
            html.cardContainer.innerHTML += createCard(country);
        })
    })
...
html.btnPopulation.addEventListener("click", function() {
    ...
    promise.then(function(dataFromFile) {
        let result = dataFromFile.filter(x => x.population >= 30000000);
        result.forEach(country => {
            html.cardContainer.innerHTML += createCard(country);
        })
    })
